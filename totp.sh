...
hex_pack() {
  if type xxd >/dev/null 2>/dev/null; then
...
gen_token() {
  ...
  case "${server}" in
    ...
    google | github | gitlab | bitbucket)
      ...
      local token=$(( ((0x${hexes}) & 0x7FFFFFFF) % $[10 ** TOTP_NUM_DIGITS] ))
      # Pad the token number with leading zeros if needed.
      printf "%0${TOTP_NUM_DIGITS}d\n" "${token}" >&3
      ;;
    *)
...
gen_token "${TOTP_SECRET}" "${TOTP_SERVER}" "${TOTP_PERIOD}" "${TOTP_NUM_DIGITS}"
